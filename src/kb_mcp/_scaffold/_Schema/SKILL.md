...
- **Images *and video* are searchable by their *visual content*, not just text-in-image or transcript.** The server CLIP-embeds every Evidence image, and samples **per-keyframe** vectors across each video (scene-aware, duration-scaled, capped), so `find` matches a photo — or a specific video *moment* — by what it depicts even when there's no text (a photo of water damage; a whiteboard 40 minutes into a 2-hour meeting) — OCR/transcripts can't do this. A purely-visual hit shows a `clip_score` signal and no text excerpt; a **video** visual hit *also* carries **`clip_match_at`** (e.g. `"14:32"`) and `signals.clip_frame_ts` (seconds) — the timestamp of the matching keyframe, so you point Hugo at the exact moment, not the whole recording. Still a first-class media hit (offer the original). This is an embedding match (measurement), not a generated description — so it stays in-bounds: *you* still describe/interpret the image/frame on the original.
