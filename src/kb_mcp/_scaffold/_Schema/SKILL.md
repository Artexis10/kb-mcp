...
- **Images are searchable by their *visual content*, not just text-in-image.** The server CLIP-embeds every Evidence image, so `find` matches a photo by what it depicts even when it contains no text (a photo of water damage, a location, an object) — OCR can't do this. A purely-visual hit shows a `clip_score` signal and no text excerpt; it's still a first-class media hit (offer the original). This is an embedding match (measurement), not a generated description — so it stays in-bounds: *you* still describe/interpret the image on the original.
