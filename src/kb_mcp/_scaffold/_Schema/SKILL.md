...
- **Media hits in `find` are first-class.** When a `find` result is an extracted media sidecar it carries `media_type` (audio/video/image/pdf) and `media_file` (a vault-relative pointer to the original binary). Treat the *file* as the result and the matched transcript/OCR snippet as the "why" — offer to pull the original via `mint_download_token` so you can view/analyze it directly (the transcript is the search index; your multimodal read of the original is the comprehension).
