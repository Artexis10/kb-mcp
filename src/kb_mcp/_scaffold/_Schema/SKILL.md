...
| **preserve** | Capture a **text** factual artifact for an incident scope. Binaries (PDF / image / any file) go via the out-of-band `/upload` endpoint (`mint_upload_token` on web) — there is no base64 path through the model | `Evidence/<scope>/` |
