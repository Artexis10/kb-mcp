...
- **Binaries go out-of-band — never inline through a tool argument.** **Transcribe** what's relevant into the note/evidence *text* (that's the queryable part), and deliver the *original file* separately. On **claude.ai web** this is hands-off — call **`mint_upload_token`** for a short-lived `{token, upload_url}`, then have the code sandbox multipart-`curl` the user's **attached** files to `upload_url` (no inline bytes, no pasted secret; see `references/operations.md` § preserve). **Searchable binaries are automatic:** the server itself transcribes audio/video (Whisper), OCRs images (Tesseract), reads PDFs (pymupdf), extracts office/web documents (docx/xlsx/pptx/html via MarkItDown; txt/eml/ics via native parsers), and CLIP-embeds images *and* per-keyframe video frames for visual search — all server-side after upload (ASR + CLIP on the GPU; OCR, PDF, and document extraction on CPU), then fills an embedded sidecar — so *any* upload (phone, curl, desk, sandbox) becomes findable by its content with no action from you. You *may* still pass a `text` form field to supply your own extraction (e.g. a richer image description from your vision, or a textless-photo caption the server can't produce) — it takes precedence and skips the server pass. Fallbacks: hand Hugo the prefilled `https://kb.substratesystems.io/upload?scope=…&category=…` link, or a desk-side Obsidian Sync drop into `Evidence/<scope>/<category>/`. `preserve` and the other write tools take **text only**; they reject inline byte blobs (`BINARY_BLOB_REJECTED`). The bytes never pass through the model — full workflow: `references/operations.md` § preserve.
...
- **Media hits in `find` are first-class.** When a `find` result is an extracted media sidecar it carries `media_type` (audio/video/image/pdf/docx/xlsx/pptx/html/text/email/calendar) and `media_file` (a vault-relative pointer to the original binary). Treat the *file* as the result and the matched transcript/OCR snippet as the "why" — offer to pull the original via `mint_download_token` so you can view/analyze it directly (the transcript is the search index; your multimodal read of the original is the comprehension).
- **Images *and video* are searchable by their *visual content*, not just text-in-image or transcript.** The server CLIP-embeds every image under `Knowledge Base/`, and samples **per-keyframe** vectors across each video (scene-aware, duration-scaled, capped), so `find` matches a photo — or a specific video *moment* — by what it depicts even when there's no text (a photo of water damage; a whiteboard 40 minutes into a 2-hour meeting) — OCR/transcripts can't do this. A purely-visual hit shows a `clip_score` signal and no text excerpt; a **video** visual hit *also* carries **`clip_match_at`** (e.g. `"14:32"`) and `signals.clip_frame_ts` (seconds) — the timestamp of the matching keyframe, so you point Hugo at the exact moment, not the whole recording. Still a first-class media hit (offer the original). This is an embedding match (measurement), not a generated description — so it stays in-bounds: *you* still describe/interpret the image/frame on the original.
...
- **`file_types` / `exclude_file_types`** (default: all kinds) — scope results to (or drop) artifact kinds: `note`, `pdf`, `image`, `audio`, `video`, `docx`, `xlsx`, `pptx`, `html`, `text`, `email`, `calendar`, `csv`, `json`, `tsv`. A binary surfaces under its media kind via its extracted sidecar — a PDF read by pymupdf, an image OCR'd by Tesseract, audio/video transcribed by Whisper *and* CLIP-embedded (per-keyframe for video) for visual search, office/web docs (docx/xlsx/pptx/html) rendered to markdown by MarkItDown, txt/eml/ics by native parsers; a data file surfaces under its **dataset card's** `format`. Omit both to search every kind — `find` never hides a type unless you ask. (`exclude_file_types=["image","pdf"]` is the inverse: everything *but* those.)
